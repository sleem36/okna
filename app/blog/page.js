...
export default function BlogPage() {
  ...
  const posts = BLOG_LIST_SLUGS.map((slug) => allPosts.find((p) => p.slug === slug)).filter(Boolean);
            ...
            <div className="main__block">
              <div className="main__text">
                <div className="main__text-title">
                  <div className="panel__text mobile_title">{BLOG_HERO.panelText}</div>
          ...
          <div className="blogs-container">
            {posts.length === 0 && (
              <p style={{ padding: '2rem', color: '#666' }}>
                Записей пока нет. Запустите <code>npm run parse-blog</code> для загрузки статей из blog.xml.
              </p>
            )}
            {posts.map((post) => (
              <div key={post.id || post.slug} className="blog first">
                <div className="blog-img">
                  <Link href={`/blog/${encodeURIComponent(post.slug)}`}>
                    <img src={BLOG_LIST_IMAGES[post.slug] || post.featured_image || '/uploads/2023/05/Rectangle-225.png'} alt="" />
                  </Link>
                </div>
                <div className="blog-content">
                  <h2>
                    <Link href={`/blog/${encodeURIComponent(post.slug)}`}>{post.title || 'Без названия'}</Link>
                  </h2>
                  <div className="date">{formatDate(post.date)}</div>
                  {post.excerpt && <p>{post.excerpt.replace(/<[^>]+>/g, '')}</p>}
                </div>
              </div>
            ))}
          </div>
